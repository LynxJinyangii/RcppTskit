...
and API alignment across the languages.
...
which can be edited.
Only some `R` functions for expanding and editing are implemented to date,
so use `Python` or  `C/C++` for the missing `R` functionality.

## API alignment across the languages

`RcppTskit` aims to keep naming, arguments, defaults, and behaviour aligned
with upstream `tskit` APIs whenever practical.
The `RcppTskit R` API intentionally combines an `R6` object model inspired by
the `tskit Python` API with operations that map directly to the `tskit C` API.
For example, `tc$node_table_add_row()` maps to `tsk_node_table_add_row()`
without recreating Python's nested `tc.nodes.add_row()` object model.
The `RcppTskit C++` API closely mirrors the `tskit C` API functions and
semantics, but is deliberately `R` oriented.
...
# Low-level R function, which calls the C++ function
